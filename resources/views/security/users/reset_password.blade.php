            ...
            <div class="card">
                <div class="card-header">
                    <div class="float-left">
                        <h4 class="header-title">Reset User Password</h4>
                    </div>
                    <div class="float-right">
                        <a class="btn btn-warning text-white" href="{{route('users.index')}}" ><i class="fa fa-list"></i> view user list</a>
                    </div>
                </div>

                <div class="card-body">
                    @include('alerts._flash')
                    {{ Form::model($user, array('route' => array('users.resetPassword',$user), 'method' => 'PUT')) }}
                    @csrf
                    <div class="form-group row">
                        <div class="col-md-12">
                            <label for="name" class="col-form-label">Fullname</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{$user->name}}" required readonly>
                        </div>
                    </div>
