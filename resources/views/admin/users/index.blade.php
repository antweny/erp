                    ...
                    @foreach ($users as $user)
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a class="btn btn-primary btn-sm mr-3" href="{{route('users.edit',$user)}}" title="edit"><i class="fas fa-edit"></i></a>
                                    <form class="form-delete" method="post" action="{{route('users.destroy',$user)}}">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit" class="btn btn-danger btm-sm" onclick="return confirm('Delete {{$user->name}} user?')" title="Delete"><i class="fa fa-trash-alt"></i></button>
                                    </form>
                                </div>
                            </td>
                        </tr>
